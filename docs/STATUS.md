...
- Version: 0.12.0
- Last iteration: plan 03 step A (rotate by 90° and flip).
- Active plan: [plans/03-rotate-flip-paint.md](plans/03-rotate-flip-paint.md). Next: step B (free-angle rotation), then step C (paint to transparency). Plans 01 and 02 are complete apart from their manual checks.
...
| `image_lab/app.py` | Main window: menus (File, Edit, Image) and toolbar with the split Save button and the rotate and flip actions (image-only actions disabled until load), status bar, drag-and-drop, open, save and color dialogs, error boxes | `MainWindow` (`load_path`, `save_to`, `quick_save`, `copy_image`, `paste_image`, `edited_image`, `export_for_drag`, `start_drag_out`, `set_fill`, `choose_fill`, `undo`, `redo`, `reset_edits`, `rotate_left`, `rotate_right`, `flip_horizontal`, `flip_vertical`, `history`), `status_text`, `fill_label`, `dropped_image_path`, `SAVE_FILTERS`, `LOAD_ERRORS`, `COLOR_DIALOG_OPTIONS` |
...
| `image_lab/model.py` | Edge model, orientation, and all geometry: output box, orientation matrix and size, the export path (Qt-free) | `Edges`, `output_box`, `output_size`, `visible_rect`, `adjust_edge`, `apply_edges`, `Transform`, `IDENTITY`, `rotated`, `flipped_h`, `flipped_v`, `transformed_size`, `affine`, `invert`, `map_point`, `view_to_source`, `apply_transform`, `rotate_edges`, `flip_edges_h`, `flip_edges_v`, `render`, `TRANSPARENT`, `SIDES` |
