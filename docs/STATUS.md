...
- Version: 0.13.0
- Last iteration: plan 03 step B (free-angle rotation).
- Active plan: [plans/03-rotate-flip-paint.md](plans/03-rotate-flip-paint.md). Next: step C (paint to transparency). Plans 01 and 02 are complete apart from their manual checks.
...
| C. Paint to transparency (erase and restore, 50% preview) | planned | |
...
| `image_lab/app.py` | Main window: menus (File, Edit, Image) and toolbar with the split Save button, the rotate and flip actions and the angle box (image-only actions disabled until load), status bar, drag-and-drop, open, save and color dialogs, error boxes | `MainWindow` (`load_path`, `save_to`, `quick_save`, `copy_image`, `paste_image`, `edited_image`, `export_for_drag`, `start_drag_out`, `set_fill`, `choose_fill`, `undo`, `redo`, `reset_edits`, `rotate_left`, `rotate_right`, `flip_horizontal`, `flip_vertical`, `set_angle`, `angle_box`, `history`), `status_text`, `fill_label`, `dropped_image_path`, `SAVE_FILTERS`, `LOAD_ERRORS`, `COLOR_DIALOG_OPTIONS` |
| `image_lab/canvas.py` | Draws the output box (checkerboard, fill over the padding, visible image part through the orientation matrix, border, handles); hover (resize cursors on edges, open hand inside), hit testing, edge dragging with frozen view, refit on release, drag-out request | `ImageCanvas` (`set_image`, `reset_edges`, `set_edges`, `set_transform`, `transform`, `set_fill`, `is_dragging`, `editFinished`, `dragOutRequested`, `edges`, `fill`, `scale`, `origin`, `hovered_edge`, `edgesChanged`), `hit_edge` |
...
| `image_lab/history.py` | Undo/redo stack of edit states: edges, fill, orientation (Qt-free) | `EditState`, `History` |
| `image_lab/model.py` | Edge model, orientation, and all geometry: output box, orientation matrix and size, the export path (Qt-free) | `Edges`, `output_box`, `output_size`, `visible_rect`, `adjust_edge`, `clamp_edges`, `apply_edges`, `Transform`, `IDENTITY`, `rotated`, `flipped_h`, `flipped_v`, `transformed_size`, `affine`, `invert`, `map_point`, `view_to_source`, `apply_transform`, `rotate_edges`, `flip_edges_h`, `flip_edges_v`, `render`, `TRANSPARENT`, `SIDES` |
...
| `tests/test_files.py` | Loading: RGBA conversion, transparency, JPEG, EXIF orientation, first GIF frame, bad and missing files. Saving: `ensure_extension`, PNG round trip, JPEG and BMP white flattening, WebP alpha, fill color, unsupported extension, orientation before edges, source untouched; `next_free_path`; `png_bytes` round trip |
| `tests/test_history.py` | `History`: empty state, round trip, redo cleared by push, duplicate pushes ignored, fill and orientation as their own steps, reset |
| `tests/test_model.py` | Geometry helpers; `apply_edges` with zero edges, pad and crop on each side, mixed edits, fill color, RGB input; `adjust_edge` clamping (including opposite crop and opposite pad) and symmetric moves; `Transform` angle normalization, view operation composition, sizes at right angles, `apply_transform` against Pillow transposes, `affine` against pixels, `view_to_source` inverse, edges following turns and flips, `render` in view pixels; free-angle size, Pillow affine convention, transparent corners, RGB kept under transparency, fill versus corners, `clamp_edges` |
