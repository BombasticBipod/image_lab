...
- Version: 0.14.0
- Last iteration: plan 03 step C (paint to transparency). All three steps of plan 03 are done.
- Active plan: [plans/03-rotate-flip-paint.md](plans/03-rotate-flip-paint.md). Complete apart from the manual checks listed below. Plans 01 and 02 are also complete apart from their manual checks. Waiting for the next plan from the user.
...
| `tests/test_files.py` | Loading: RGBA conversion, transparency, JPEG, EXIF orientation, first GIF frame, bad and missing files. Saving: `ensure_extension`, PNG round trip, JPEG and BMP white flattening, WebP alpha, fill color, unsupported extension, orientation before edges, painted pixels in PNG (alpha 0, RGB kept) and BMP (white), source untouched; `next_free_path`; `png_bytes` round trip |
